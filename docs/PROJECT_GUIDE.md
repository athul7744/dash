...
- **Link preview cards** — `src/lib/notes/link-embed.ts` + `src/lib/notes/editor/link-embed-insert.ts` + `src/components/notes/editor/LinkEmbedNode.tsx`
  ...
  - A card carries its own hover actions (open, copy, edit, remove). Editing offers the address and the title, and they behave differently on purpose (`mergeLinkEmbedEdit`): the title is a label, yours to set and kept; changing the address makes everything else stale, so description, host and thumbnail are re-read (`refetchLinkEmbed`) and only a title you typed yourself survives. A relabel touches no network. A replaced thumbnail is deleted once the new attrs are saved — by id (`deleteAttachmentById`), not by block, since the replacement is owned by the same block and `deleteEntityAttachments` would take it too. Left behind it would still be listed in the page's Files rail, which matches block-owned files whose block is alive, and the orphan sweep would not reclaim it: that sweeps Storage objects with no live row, and the row is live. The editor's link toolbar deliberately passes over one (`linkAt`): that toolbar edits a link *mark*, and a card is a node — there is no mark to change, and its rendered text spans a block, so the end position the toolbar infers from text length ran past the end of the document and `removeMark` threw
