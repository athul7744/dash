...
- **A surface waits for its data once.** A watched query re-runs whenever a table it reads is written, and its `isLoading` goes back up while it does — so gating a screen on the raw flag sends it back to a skeleton every time anything underneath changes, and returning to it looks like opening it for the first time. `src/hooks/use-settled.ts` turns that into "has loaded at least once"; use it for any surface-level gate. The notes overview did this wrong *and* paid for three full-table `COUNT(*)` scans (one over `blocks`, re-run on every keystroke's write) to produce a loading flag whose numbers nothing read
