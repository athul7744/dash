...
export function NotesOverviewGallerySkeleton() {
  return (
    <SkeletonWave className="grid grid-cols-2 gap-3 sm:gap-4 sm:[grid-template-columns:repeat(auto-fill,minmax(15.5rem,1fr))]">
...
export function NotesOverviewSpineSkeleton({ pins = false }: { pins?: boolean }) {
  ...
  return (
    <div className="relative">
      ...
      <SkeletonWave className="space-y-5">
...
function NotesNavigationRailSectionSkeleton({ itemWidths }: { itemWidths: string[] }) {
  return (
    <div className="space-y-2.5">
      ...
      <div className="space-y-1 overflow-hidden pl-8">
        {itemWidths.map((widthClassName, index) => (
          <div key={index} className="flex items-center gap-2.5 px-3 py-1.5">
            <Skeleton className="h-6 w-6 shrink-0 rounded-lg" />
            <Skeleton className={`h-3.5 ${widthClassName}`} />
          </div>
        ))}
      </div>
    </div>
...
export function NotesEditorMainSkeleton() {
  return (
    <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-1 gap-y-1.5 pt-1 md:gap-x-2 sm:h-full sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:grid-rows-[auto_auto_auto_minmax(0,1fr)]">
      ...
      {/* Title */}
      <Skeleton className="col-start-1 h-10 w-3/4 sm:col-start-2 md:h-14" />
...
export function NotesDetailsRailSkeleton({ showHeader = true }: NotesDetailsRailSkeletonProps) {
  return (
    <div className="space-y-3 py-1">
      ...
      {/* Overview / Connections tab switcher */}
      <div className="flex gap-0.5 border-b border-border/30 pb-1.5">
        <Skeleton className="h-7 w-24 rounded-md" />
        <Skeleton className="h-7 w-28 rounded-md" />
      </div>
      <div className="divide-y divide-border/60 pl-2">
...
export function NotesGraphSkeleton() {
  return (
    <div className="relative h-full min-h-[420px] w-full overflow-hidden rounded-2xl border border-border/60">
      ...
      >
        <span className="absolute left-1/2 top-[22%] h-16 w-16 -translate-x-1/2 rounded-full bg-muted-foreground/15" />
        <span className="absolute left-[26%] top-1/2 h-12 w-12 rounded-full bg-muted-foreground/15" />
        <span className="absolute right-[24%] top-[48%] h-16 w-16 rounded-full bg-muted-foreground/15" />
      ...
      <div className="absolute left-3 top-3 w-52 max-w-[46vw] space-y-3 rounded-xl border border-border/60 bg-popover/90 p-3 shadow-lg backdrop-blur-sm sm:left-4 sm:top-4 sm:w-56">
        ...
        <div className="flex items-center justify-between"><Skeleton className="h-3.5 w-20" /><Skeleton className="h-5 w-9 rounded-full" /></div>
        <div className="flex items-center justify-between"><Skeleton className="h-3.5 w-24" /><Skeleton className="h-5 w-9 rounded-full" /></div>
      ...
      <div className="absolute right-3 top-3 w-40 max-w-[42vw] space-y-2 rounded-xl border border-border/60 bg-popover/90 p-3 shadow-lg backdrop-blur-sm sm:right-4 sm:top-4 sm:w-44">
        <Skeleton className="h-3 w-16" />
        <div className="flex items-center gap-2"><Skeleton className="h-2.5 w-2.5 rounded-full" /><Skeleton className="h-3.5 w-20" /></div>
        <div className="flex items-center gap-2"><Skeleton className="h-2.5 w-2.5 rounded-full" /><Skeleton className="h-3.5 w-14" /></div>
      </div>
  ...
  if (mode === "overview") {
    return (
      <section className="space-y-6 animate-fade-slide-in">
        {/* Sticky search-bar row (search pill + mobile graph shortcut). */}
        <div className="flex items-center justify-center gap-2">
          <Skeleton className="h-10 w-full max-w-xl rounded-full" />
          <Skeleton className="h-10 w-10 shrink-0 rounded-full sm:hidden" />
        </div>
          ...
          {Array.from({ length: 2 }).map((_, index) => (
            <section key={index} className="space-y-4">
              <div className="flex items-center gap-2">
                <Skeleton className="h-4 w-4 rounded-full" />
                <Skeleton className="h-4 w-28" />
                {/* Favorites section carries the ViewSwitcher accessory pill. */}
                {index === 0 ? <Skeleton className="ml-auto h-7 w-24 rounded-full" /> : null}
              </div>
