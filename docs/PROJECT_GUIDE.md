...
- **Page banner** — `src/lib/notes/banner.ts` + `src/components/notes/page/NotePageBanner.tsx`
  ...
  - Removing or replacing a banner **clears the property and keeps the file**. Deleting it needs a "we uploaded this one" flag to avoid destroying an image the page uses elsewhere (the picker offers any image already attached), and a listed file is a smaller loss than deleted bytes. Deleting it from the rail clears the property in the same handler, and a banner whose row is gone renders nothing rather than a broken frame.
