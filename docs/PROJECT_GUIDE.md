...
- `src/components/command/CommandPaletteProvider.tsx` — the global ⌘/Ctrl+K command palette, mounted app-wide in the root layout. Runs ranked full-text search across all five entities (via `searchEntities`, see [Search](#search)) — with match highlighting and note-body snippets — and runs Go-to-app / Quick-capture / New-item commands; built on `SearchPopup`. A `kind:` filter renders as a removable chip in the input (backspace deletes the value to the colon, then the chip); a "Filter by kind" group teaches and applies it. A `tag:<name>` filter (completed from a "Filter by tag" list) bypasses ranking to list every entity carrying that tag across apps — the cross-app "under a tag" view over `entity_tags` (`useEntitiesByTag`) — with trailing text narrowing within it. Also listens for reference-chip open events (see [Cross-App Links & References](#cross-app-links--references))
