...
- `src/components/AppHeader.tsx`
  ...
  - Handles theme toggle, sync indicator, logout, and the mobile overflow menu — which now carries only what the page passes as `mobileMenuItems`, and isn't rendered at all when there are none. **Settings moved to the switcher FAB on mobile**, where a thumb is; the header keeps the gear on desktop. `AppSwitcher` dispatches `OPEN_SETTINGS_EVENT` and `AppHeader` listens, so the dialog (and the importer it opens) stays mounted in one place rather than the switcher carrying a second copy.
...
- `src/components/AppSwitcher.tsx`
  - App-to-app switcher used in the shell; its footer also links to the Dashboard and the vault Graph (`/graph`)
...
- `src/components/AppBootSkeleton.tsx`
  - Cold-start fallback (shown by `powersync-provider` while the local DB opens) that picks the route-shaped skeleton by pathname (`/tracker/<view>` for the tracker; `/graph`, `/notes/<id>` and bare `/notes` for the graph, editor and notes overview), so a refresh boots into the matching skeleton with no blank gap
...
- `src/app/notes/layout.tsx` + `src/app/notes/[[...slug]]/page.tsx` — the notes shell (path-based `/notes`, `/notes/<id>`; see [Notes App Structure](#notes-app-structure))
...
- `src/app/graph/page.tsx` — the vault graph (see [Notes App Structure](#notes-app-structure))
- `src/app/day/[date]/page.tsx` — one calendar day (see [The Day surface](#the-day-surface))
- `src/app/trash/page.tsx` — the cross-app trash
...
- `src/components/command/EntityPopup.tsx` — opens any task/bookmark/quote/event in a blurred modal by reusing that app's own card (looked up live by id; notes deep-link to `/notes/<id>`). Used by the palette, the dashboard hero nudge, and reference-chip clicks. It closes itself on either thing a card can do out from under it: the route changing (following an event title or a reference chip — an external link changes no path and leaves it open), and its entity ceasing to resolve, which is what deleting from the card's own menu does. Both lookups exclude trashed rows, like every list query
...
- `src/lib/shared/item-colors.ts` — the 23 assignable color names shared by tags, tracker activities, and moods, with swatch, tag badge, grid cell, and chart/graph colors; stored names stay stable across all three
- Collection cards in Notes, Tasks, Quotes, Bookmarks, and Events, Tracker week widgets, Graph stages, Trash rows and toolbar, Day summary and entity rows, and Event detail panels use `bg-muted/40 dark:bg-card/80` with their own borders and layouts. `EntityPopup` reuses the Task, Bookmark, Quote, and Event cards on a solid `bg-background` backing. Task subtask panels use `bg-muted/50` in light mode and `bg-black/20` in dark mode. Dashboard sections remain unframed. Tracker week hour headers use `bg-muted` like the sticky Mood and Day columns. Activity cell fills keep small labels legible in both themes, while year-grid labels and other informational metadata use the full `text-muted-foreground` rather than reduced-opacity variants.
- Light-mode `--background` in `src/app/globals.css` is `oklch(0.985 0 0)`. `--field-surface` distinguishes Dashboard and Notes search triggers and the command/search popup inputs from the page and card fills. Graph search uses the same surface; small fields, inline card editors, document titles, and the rich-text editor retain their own backgrounds.
- `src/lib/dashboard/hero-action.ts` — pure rule+weighted-score picker (`chooseHeroAction`) for the hero's contextual nudge
- `src/lib/tasks/colors.ts` — tag palette and class helpers backed by the shared item colors
...
- `src/lib/bookmarks/bookmarks.ts` / `bookmarks/daily.ts` / `bookmarks/metadata.ts` / `bookmarks/fetch-metadata.ts` / `bookmarks/ssrf.ts` — bookmark CRUD over the `kind:"bookmark"` system page, the unread-weighted `pickDailyBookmark`, the pure `parseMetadataHtml`, the SSRF host guard shared by both proxy routes, and `refreshBookmarkMetadata` (fills the title and stores the og:image as the bookmark's preview attachment, fetched through `fetchRemoteImage` to dodge CORS; `refreshBookmarkTitle` is the title+image wrapper). Two server routes back it: `src/app/api/bookmark-metadata/route.ts` (returns `{title,description,image,host}`; it streams until `</head>` or 1 MB — the cap only binds on pages whose head is mostly script). When scraping leaves the title or image empty it falls back to **oEmbed** (`src/lib/bookmarks/oembed.ts`), which is the only thing that works for a page rendered in the browser: YouTube serves a fetcher a shell with no og tags and an empty `<title>` whatever User-Agent it is asked with, so a video link had no title, description or image. An endpoint the page advertises (`<link type="application/json+oembed">`) is preferred, with a small table for providers like YouTube that advertise nothing. A discovered endpoint is chosen by the page — the one attacker-controlled URL this server fetches — so it must be **on the page's own host**, and still goes through the blocked-host guard: that guard alone would follow a redirect from a public host to a private one, and cross-host providers are the ones in the table, which we vetted. What counts as a gap is the point: `parseMetadataHtml` reports whether each value is the page's own answer (`og:`) or a site-level fallback (`<title>`, `name="description"`), because YouTube's shell says " - YouTube" and describes YouTube — non-empty and useless. So an `og:` value always wins, a site-level title loses to the provider's, and a site-level description is dropped only when the page said nothing about *itself* at all — no og:title and no og:image, the shell signature — since otherwise a `name="description"` is very likely the real thing (oEmbed carries none, and nothing beats boilerplate). A page that gave its own title and an image is never asked twice and `src/app/api/remote-image/route.ts` (streams the image bytes; shared with the notes editor's inline images)
...
- `src/lib/tracker/activities.ts` — tracker activity palette and cell/dot class helpers backed by the shared item colors
...
- `src/lib/storage/*` + `src/hooks/use-attachment-url.ts` / `use-attachment.ts` / `use-entity-image.ts` — the file-attachment layer. Bytes live in a private Supabase Storage bucket (`attachments`), never in PowerSync — only the `attachments` metadata row syncs. `attachFile(file, {pageId}|{blockId})` caches the bytes locally (`local-blob-store.ts`, OPFS with an IndexedDB fallback) and inserts a `pending` row; a caller whose owner row isn't written yet splits that in two — `storeFileBytes`, then `insertAttachmentRow` once the owner exists — because a row uploaded ahead of its parent is refused by the foreign key and dropped, leaving the file on one device (`discardStoredBytes` drops bytes that never got a row, which is the whole cleanup since nothing else refers to them); `attachment-sync.ts` is a `db.onChange` reconciler (the sole bytes maintainer) that uploads pending rows (`UPLOAD_CONCURRENCY` at a time — independent requests that would otherwise queue one behind another on a vault import), resolves a view URL (local cache → download-and-cache), and sweeps Storage objects with no live row — the offline-safe cascade-delete backstop. `paths.ts` holds the pure path/validation/orphan helpers, and `remote-image.ts` is the one client path for pulling a remote image in through the `/api/remote-image` proxy: `fetchRemoteImage` for the bytes, and **`fetchPreviewImage`** — shared by a link card's thumbnail and a bookmark's preview — for a stored `og:image`, because all three steps have to agree. It bounds the image to 640px (`downscale.ts`) since an og:image is routinely 2400px for a card that draws it a tenth that wide, and the browser's one-pass reduction at that ratio uses its cheap filter, which makes a banner carrying text look ragged; stepping down by halves once, on the way in, is both sharper and a fraction of the bytes to sync. SVG and GIF are left alone (a vector already scales; a canvas keeps a GIF's first frame and silently drops the rest), and every failure path returns the original blob, so the worst case is the picture we already had. Re-encoding changes the format and `extFor` trusts a file name's extension over the mime type, so the original extension is dropped rather than left lying about the bytes. `blob-preview.ts` is a bounded in-memory cache (primed by `attachFile`, evicted oldest-first on an entry and byte budget) holding both the blob and **one stable object URL per attachment id**. It exists for two reasons, and a flicker-free paste needs both: reading bytes back out of the local store is async, so `NoteImageView` reads the URL *during render* rather than resolving in an effect (which paints a placeholder for a frame first); and the editor replaces its whole document whenever rows change — which rebuilds every `<img>` — so the src has to be a string the browser has already decoded. A URL minted per mount re-fetches and re-decodes on each of those rebuilds, which is visible. A shared URL needs shared ownership, so consumers `acquirePreviewUrl`/`releasePreviewUrl` and it is revoked only once nothing holds it *and* it has left the cache, deferred a tick so a remount that re-acquires cancels it (React's development double-mount does exactly that). `resolveUrl` still mints its own URL from the cached blob for callers that own their lifetime, and `useAttachmentUrl` swaps a re-resolved URL in *before* revoking the old one, so an upload finishing never blanks a live image. `deleteEntityAttachments(id, ctx?)` joins an entity's delete fan-out (see `deleteBookmark`, the block persister's DELETE, and `deleteNotePage`). Lookups: `useAttachmentUrl` (a row → a viewable URL), `useAttachment` (one row by id — how an inline image resolves, since it carries an attachment id rather than an owning entity id), `useImageSource` (the whole chain — preview → id → row — shared by every surface that renders a stored image from an id alone), `useEntityImages(ids)` (the first image for each of a list's entities, batched — one query for every card on screen, asking `block_id` and `page_id` as two branches of a `UNION ALL` so both stay indexed) and `useEntityImage` (the same for one entity). Surfaces: the notes editor's inline images and page banners (see [Notes App Structure](#notes-app-structure)), the Notes details rail (`NotesDetailsRail.tsx` — upload, image thumbnails, download, delete; it lists every file on the page, so a block-owned inline image shows as "In the page" and can only be removed by deleting the image itself), and bookmark preview images (the list calls `useEntityImages` and hands each card its row; `BookmarkCard` only resolves the URL)
- `src/lib/shared/trash.ts` + `src/hooks/use-trash.ts` / `use-trash-action.ts` + `src/components/toast/ToastProvider.tsx` — the shared soft-delete (trash) layer. Deleting a bookmark/quote/event/note-page/task stamps a reversible marker — `blocks.deleted_at` / `pages.deleted_at`, or `tasks.state='trashed'` — instead of destroying rows; relationships (edges/tags/attachments/occurrences) survive so a restore is lossless. `softDeleteEntity`/`restoreEntity` flip the marker (bumping `updated_at` so the search reconciler catches it) and cascade the occurrence log; `purgeEntity` delegates to the existing hard-delete functions for the real fan-out. Every list/search query gains `deleted_at IS NULL` (the central `useSystemPageBlocks` seam covers bookmarks/quotes/events). `useTrashAction` (via the app-wide `ToastProvider`, mounted in `layout.tsx`) is the delete→undo-toast entry point every card calls; `useTrashedItems` feeds the global `/trash` page, reachable from the AppSwitcher footer + ⌘K palette like Graph. Its bulk controls (Select, Restore all, Delete all, and Restore/Delete over a selection) sit in a toolbar above the list rather than the header's `actions` slot, which renders on desktop only — a phone had no way to empty the Trash. `purgeEntities`/`restoreEntities` run a list one at a time, since each purge is a fan-out across several tables; the caller passes a snapshot because the reactive list shrinks as they land, and the selection is read off the live list so an item removed on another device drops out instead of being acted on twice. Note *blocks* are excluded (the editor owns them); trash covers note *pages*.
...
- `src/lib/notes/banner.ts` — a page's banner image: reading it out of `pages.properties`, writing it back, storing a picked file or a remote URL, and the drag-to-crop maths — see [Notes App Structure](#notes-app-structure)
- `src/lib/notes/import/*` — the Logseq/markdown importer: `logseq-normalize` (Logseq syntax → plain markdown, pure), `page-properties` (`key:: value` reading and reference unwrapping), `property-mapping` / `tag-mapping` (censuses + suggested actions), `title-allocator` (filename → unique title), `asset-index` (image reference → picked file), `pick-markdown-files`, `scan-import` (the dry run), `run-import` (the writes). Everything but the last two is pure — see [Notes App Structure](#notes-app-structure)
- `src/lib/notes/editor/*` — the single-document editor's non-React core: `block-schema` (the `block` wrapper node + `blockContent` grouping), `block-document` (assemble rows → one doc / decompose doc → rows, incl. legacy `taskList` → task-block migration), `block-diff` (churn-minimal rank/write diff), `block-persister` (debounced save + remote reconcile), `block-commands` (native split/merge/indent/outdent), `block-id-plugin` (stable block ids), `block-normalize` (one-content-node-per-block invariant), `slash-single` (slash detect/apply), `markdown-paste` (parse pasted raw markdown text → block/`taskLine` nodes; detection + insertion for the editor's `handlePaste`, incl. `pasteUrlAsLink` which links a pasted bare URL and `insertBlockNodes`, the shared slice insert), `image-insert` / `image-adopt` (inline images — see [Notes App Structure](#notes-app-structure)), and `extensions` (the assembled Tiptap extension list)
...
- `src/lib/powersync/AppSchema.ts` — local schema definition, including each table's **local indexes**. These exist only in this device's SQLite file: PowerSync builds them over the same `json_extract` expression its views select, so a query written against a view uses them, and adding one needs no Supabase column, sync-rule change or migration — the next launch rebuilds it. An index earns its place when its predicate is *selective* and its query runs on a screen opened often or re-runs on every write to its table; a filter that matches most rows gains nothing, because the planner won't choose the index. Two traps: an `OR` across two columns makes SQLite abandon both indexes and scan (ask the two columns as branches of a `UNION ALL` instead — see `useEntityImages`), and every index is paid for on each insert
...
- `src/lib/powersync/SupabaseConnector.ts` — sync connector implementation. `uploadData` pulls up to `CRUD_BATCH_LIMIT` (1000) CRUD entries per call — it is called again while anything is left, so this only decides how many round trips a bulk write costs — collapses them to one net op per (table, id), and executes them **per table** in `UPSERT_CHUNK`/`DELETE_CHUNK` sized requests (a thousand rows of block content is several MB, and PostgREST puts delete ids in the URL). Grouping per table which loses the order the local writes were made in — so `upload-helpers.ts` holds the order it has to be put back into: `PUT_TABLE_ORDER` sends a parent table before anything that references it (`pages` → `blocks` → `attachments`, `tags` → `entity_tags`), `DELETE_TABLE_ORDER` the reverse. **A table with a cross-table foreign key must be listed**: anything absent sorts last, alphabetically, and a child row that reaches Supabase before its parent is refused (SQLSTATE 23503) and dropped by `upsertSkippingOrphans` — silently, since the local row is fine. That drop is a backstop for a genuine create/delete race, not a substitute for ordering. Every other failure is rethrown as an `UploadError` **carrying the server's SQLSTATE**, because the code is the only thing separating "retry this" from "this can never work": a data, integrity or access error (`isFatalResponseCode` — classes 22, 23, 42) discards the batch, since the queue is strictly ordered and one unsatisfiable op otherwise blocks every write behind it forever.
- Ordering within a table, and across batches, comes from the op order — which is the order of the local writes. So **a row must be written locally after the row it points at**: see `storeFileBytes`/`insertAttachmentRow` in the attachment layer, and the note importer, which writes its `attachments` rows after the page's own transaction.
...
- URLs are path-based: `/notes` (overview), `/notes/<id>` (a note) — no `?page=`/`?view=` query params or fallbacks. The graph is not one of them: it maps every app, so it lives at `/graph` (`src/app/graph/page.tsx`), outside the notes shell whose rails it never shows. The route is an optional catch-all `src/app/notes/[[...slug]]/page.tsx`, but the page itself returns `null`.
- **A surface waits for its data once.** A watched query re-runs whenever a table it reads is written, and reports `isFetching` while it does; `isLoading` means "no result yet" and goes back up when the query is mounted afresh, which a route change does. Gating a screen on either raw flag sends it back to a skeleton while it already has rows on screen, and returning to it looks like opening it for the first time. `src/hooks/use-settled.ts` turns that into "has loaded at least once"; use it for any surface-level gate (`useSystemPageBlocks` feeds it `isLoading || isFetching`, which is the pair to latch on). The notes overview did this wrong *and* paid for three full-table `COUNT(*)` scans (one over `blocks`, re-run on every keystroke's write) to produce a loading flag whose numbers nothing read
- **A page's summary costs a block read.** `NOTE_PAGE_SELECT` carries a correlated subquery pulling the first block's JSON per page, which is right for a handful of cards and wrong for every page in the vault. `useAllNotePages` (always mounted — it feeds the wikilink title index) uses `NOTE_PAGE_COLUMNS` without it; `useAllNotePagesWithPreview(enabled)` is the summary-bearing list, taken only by the page-search popup while it is open and by the command palette, which only mounts when open
- **A screen you come back to should already be there.** Leaving an app unmounts its screen and closes every query it was watching, so returning rebuilds them from nothing — a skeleton on every navigation, which is the difference between an app and a website. `src/hooks/use-cached-query.ts` keeps the rows each query last settled on outside React, keyed by the query and its parameters, and a remount paints them while the real query re-runs. Use it for a screen-level list, not for a lookup by id. Its `isLoading` means "nothing to show", so it is already false when something is remembered — which is what `useSettled` should be given. Keeping the `WatchedQuery` itself alive between mounts was the other option and was not taken: a live query re-runs on every write to the tables it reads, so every screen ever visited would go on querying in the background. Remembered rows are only ever shown to the query that produced them, and only while that query has no result of its own; `clearRememberedQueries()` runs on a local reset and on sign-out, where the rows describe a database or an account that is no longer there. The other half of the same problem was the user id: it is cached, but `useCurrentUserId` resolved it through a promise, so every mount rendered once with `null` and any screen deriving a system page from it had nothing to query on its first render — `cachedUserIdOrNull()` makes it synchronous once anything has resolved it
- **One query for a list, not one per row.** A hook that reads a row's companion data — its tags, its preview image — belongs on the list, keyed by every visible id, not inside the card. Twenty cards each running their own lookup is twenty live queries that all re-run whenever anything writes to that table. `useEntityTags(ids)` and `useEntityImages(ids)` both return a `Map` the list indexes into; measured, batching the bookmark cards' image lookup took 20 cards from 13 ms to 0.04 ms
...
- **Markdown / Logseq import** — `src/lib/notes/import/*` + `src/components/notes/page/ImportLogseqDialog.tsx`
  - Row rendering lives in `ImportMappingRows.tsx`: one set of cells placed by row and column, so a phone gets a stacked layout (title, then path, then detail; the action select full-width) and a wide screen gets fixed columns that line up down a long list — and each closed select renders its own label (base-ui's `Select.Value` shows the raw *value* otherwise, surfacing encodings like `create:text`). Actions read as sentences with an icon per kind — built-in, existing, create, ignore — and a row whose action needs no name shows what will happen instead of an empty input.
  - Reached from **Settings → Data**, and only there: Settings is reachable on every surface (the header gear on desktop, the switcher FAB on mobile), so an item of its own would be both a duplicate entry point and a second mount of the dialog. It sits in Settings rather than the notes header because that header already carries four labelled actions — an icon-only fifth read as an afterthought. Settings closes when it opens, so the dialog is mounted in `AppHeader` beside `SettingsDialog` (and only while open, since it queries every page, definition and tag) — mounting it inside Settings would unmount it mid-import. Four steps: **pick** a folder (`webkitdirectory`, so `webkitRelativePath` lets an image reference find its asset) or files → **scan** → **review** → **map** → run. Picking `pages/` instead of the vault root is an easy and quiet mistake — every note imports and every local image is simply absent — so the review step says so when the pick holds no `assets/` file and a banner failed to resolve.
  - **The scan is a real dry run.** Every file is read, normalized and parsed before anything is shown, so a per-file status (`ready` / `journal` / `already` / `notMarkdown` / `empty` / `failed`) is a fact rather than a guess from the extension. Only the verdict is kept — holding every parsed document would balloon memory on a large vault, and re-parsing at write time is the cheap half. `scan-import.ts`.
  - **Mapping runs after selection**, deliberately: it decides which tags and `property_definitions` get created, and those must reflect the files actually chosen. `property-mapping.ts` and `tag-mapping.ts` census the keys and values and *suggest* an action per row; the dialog shows the census and every row is overridable, so nothing is decided silently at write time. A key matching an existing definition (or a value matching an existing tag) maps onto it rather than making a twin, which is also what makes a second import idempotent — and the action carries that definition's **type**, so a `date::` mapped onto a Date property lands as an ISO day rather than a string the panel can't render. Folder tags never appear as rows, so they go through `tagNameFor` (the same hyphenation the screen insists on) and the checkbox says so. `tagDecisionProblem` blocks the run on either bad name a created tag can have: **cleared** (`ensureTagIdsByName` skips blanks, so it would create nothing and every page carrying the value would lose its tag in silence) or **spaced** (the `tag:<name>` search prefix stops at a space, so the tag it creates can never be found that way). Several of a real vault's values are page titles, which have spaces by nature, so the footer offers to hyphenate every spaced name at once — the block is a decision to make, not the same edit five times. The offending row carries the amber input and the suggestion.
  - **The Logseq layer is text-to-text.** `logseq-normalize.ts` rewrites what Logseq adds on top of markdown — leading `key:: value` page properties split off, block-level bookkeeping (`id::`, `collapsed::`, drawers) stripped, `{{embed [[X]]}}` → `[[X]]`, `- TODO/DONE foo` → `- [ ]`/`- [x]` — then hands plain markdown to `markdownToBlockNodes`. **No Logseq-specific structural code exists**: `listItemBlock` (`markdown-paste.ts:161-196`) already maps an outliner correctly, nesting a bullet's extra paragraphs as child blocks. What can't be translated (`{{query}}`, `((block refs))`, org blocks) is left as text and *counted*, and the count is what the picker shows beside a file.
  - **Titles come from the basename**, `___` → `/` and percent-decoded (`title-allocator.ts`): a Logseq `[[link]]` targets the basename, and the title is the key `reconcileEntityRefs` matches on, so a path-shaped title would break every link. Titles are globally unique and `createNotePage` *throws* on a collision after a full table scan, so the import prefetches every title once and allocates in memory — which also settles clashes between two files in the same batch. **The review step previews that**: it runs the same allocator over the selection in import order, so a file that will be renamed says so (and whether it clashed with an existing page or with another file in the batch) rather than being suffixed silently at write time.
  - **Writes are one transaction per file** (`run-import.ts` → `createNotePageFromBlockNodes` in `notes.ts`): page, blocks and tags together, so a malformed file costs only itself. It passes `deferEdges`, because reconciling inside the transaction reads every page title *per block* to build edges the second pass replaces anyway. Nothing awaiting I/O runs inside a write lock. Block ids are stamped *before* anything is written (`stampBlockIds`), because an image's file is stored against the block that owns it. The bytes are cached before the transaction and the `attachments` rows written after it, so no row can reach the server ahead of its block; a failed page write just drops the cached bytes, which nothing references and nothing has uploaded. `properties.kind` must stay absent or the page is hidden from `/notes`, search, graph and trash.
  - **Links resolve in a second pass.** `reconcileEntityRefs` silently drops a `[[Title]]` whose page doesn't exist yet, so a file linking to one imported later would lose the edge. Once every page is in, each imported block is reconciled again against a title index built once (`buildTitleIndex`, `src/lib/links/links.ts`) — without it, reconciling reads all page titles *per block*.
  - **Images come from two places.** A reference that resolves to a picked file is stored from disk; one that lives at a URL is fetched through `/api/remote-image` when the "download remote images" box is ticked (on by default, with the count from the scan, since a vault's images are often mostly remote). Left hotlinked they need the network on every read and are only pulled in page by page as each is opened by the editor's adopt pass. A downloaded image keeps its `src` so the original URL survives a markdown export; a local one drops it. Anything that can't be fetched keeps its hotlink, and the adopt pass can still try later.
  - **Banners come across as banners.** `banner::` maps to the page banner (a page attachment whose id lands in `properties.banner`) and `banner-align::` to its crop — `bannerAlignPercent` reads the percentages a real vault holds as well as CSS keywords, so a page arrives cropped the way it was left. A remote `banner::` goes through the same download step as a remote image and counts toward its total. The scan resolves each ref up front (`banner: "local" | "remote" | "missing"`), because a vault can name an asset it no longer holds: those files say `banner missing` in the review list and arrive without one, never with a `banner` key pointing at a file that was never stored.
  - Each page records `properties.importedFrom` + `importedAt`, which drives the `already imported` status; `properties.importedFrontmatter` holds unmapped keys (stored, not shown). The whole batch is undoable, and the undo **outlives the dialog** — you only know whether an import worked after opening a few pages. Every page is stamped with one `properties.importedBatch` per run (per-file timestamps differ by milliseconds and are no use for finding "the last import"), so `undo-import.ts` rebuilds the batch by query rather than holding ids in component state. Three affordances, same soft delete: an **Undo import** button on the done step, a 30s toast for right after closing, and **Settings → Data → "Undo last import"** showing the count and age. All call `softDeleteEntity` per page, so blocks, tags, links and stored images survive and each page is individually restorable from `/trash` — undoing an import is itself reversible. A batch whose pages are all trashed drops out of the query, so the button can't show a stale count.
  - Dropping a `.md` onto an open note inserts it inline at the drop point — the same normalizer, then the existing `insertMarkdown` (`useSingleBlockEditor.ts` `handleDrop`).

- **Link preview cards** — `src/lib/notes/link-embed.ts` + `src/lib/notes/editor/link-embed-insert.ts` + `src/components/notes/editor/LinkEmbedNode.tsx`
  - `/embed` asks for a web address and turns it into a card: title, description, host and thumbnail. The `linkEmbed` node is an atom in the `blockContent` group, so it sits in a `block` wrapper like any other block-level content, and its attrs *are* the card — **a snapshot taken once, never re-fetched**, so a note opens the same offline as online and reads the same a year later. The thumbnail is stored as a block-owned attachment and resolves through `useImageSource`, exactly as an inline image does
  - Metadata comes from `/api/bookmark-metadata`, the same auth-gated, SSRF-guarded proxy bookmarks use, and the image through `fetchRemoteImage`. **Every lookup is allowed to fail**: no metadata, a host that won't serve its image, or no network each give a thinner card rather than none — the URL and its host are always enough to show something and to click through
  - A thumbnail goes through `fetchPreviewImage` (`src/lib/storage/remote-image.ts`), which every stored `og:image` shares with a bookmark's preview — see the attachment layer below for what it does and why
  - The write order is `image-insert.ts`'s, for the same reason: mint the block id, cache the bytes under it, insert the block, flush so the `blocks` row lands, *then* write the `attachments` row — one written ahead of its parent is refused by the server's foreign key and stranded on one device. A failed insert discards the bytes, which is the whole cleanup
  - A card degrades to its URL in plain text (so edge reconcile and markdown round-trip keep the link, not a title that points nowhere) and to `[title](url)` in markdown; its title and description join the text the search index reads, which is the only copy of the linked page held here
  - A card carries its own hover actions (open, copy, edit, remove). Editing offers the address and the title, and they behave differently on purpose (`mergeLinkEmbedEdit`): the title is a label, yours to set and kept; changing the address makes everything else stale, so description, host and thumbnail are re-read (`refetchLinkEmbed`) and only a title you typed yourself survives. A relabel touches no network. Saving leaves the block owning exactly the thumbnail it shows (`keepOnlyBlockAttachment`) — not merely the one id being replaced, so a card edited before this existed sheds its older versions too; a new address with no picture of its own clears the block instead. Left behind they would be listed in the page's Files rail, which matches block-owned files whose block is alive, and nothing else would reclaim them: the orphan sweep looks for Storage objects with *no* live row, and the rail deliberately won't delete a block-owned file, since that would strand the block pointing at it. A block that owns its files has to tidy its own history. The editor's link toolbar deliberately passes over one (`linkAt`): that toolbar edits a link *mark*, and a card is a node — there is no mark to change, and its rendered text spans a block, so the end position the toolbar infers from text length ran past the end of the document and `removeMark` threw
  - **Pasting a URL is unchanged** — it stays a link (`pasteUrlAsLink`). Embedding is deliberate, because a URL typed mid-sentence is the common case
- **Inline images** — `src/lib/notes/editor/image-insert.ts` / `image-adopt.ts` + `src/components/notes/editor/NoteImageView.tsx` + `src/hooks/use-attachment.ts`
  - The `image` node (`NotesImage`, in `NoteBlockEditorExtensions.ts`) carries an `attachmentId` attr pointing at a row in the attachment layer; `src` holds a plain URL for an image not yet stored. `NoteImageView` is the React NodeView that resolves whichever is set (`useAttachment` → `useAttachmentUrl`), shows a loading/missing/failed placeholder otherwise, and edits alt text. A blob URL must never be written into attrs — it's per-session and per-device, and attrs are persisted. The NodeView is attached in `extensions.ts` rather than on the node, so the node stays importable without dragging the db into it.
  - `insertImageFiles` is the single path behind paste (`handlePaste`, only when the clipboard carries no text — see `clipboardImageFiles`), drop (`handleDrop`, positioned with `posAtCoords`), and the `/image` slash command (`custom: "image-picker"`, handled in `SlashMenuLayer` like the date picker). It **mints the block id first**, caches the bytes under it, inserts the block carrying both ids, awaits `flushAllBlockDocumentPersisters()`, and only then writes the `attachments` row. That order is load-bearing twice over: because the `blocks` row lands immediately, removing the image produces a delete write and the persister's DELETE branch already drops the block's attachments — cleanup needs no code of its own; and because the row is written *after* the block's, the upload can't carry it to the server ahead of its parent. Nothing is recorded until the insert succeeds, so a failed write leaves no broken node and no row to undo. One image ⇒ one block (the node is block-level and `block-normalize` enforces one content node per block), which is what makes the entity-wide `deleteEntityAttachments(blockId)` correct.
  - `ImageAdopt` is a debounced pass that finds images still pointing at an `http(s)` URL (`findAdoptableImages`, pure over the doc JSON), downloads them through `fetchRemoteImage`, and records the attachment id (`adoptImage`, which takes the "point the node at it" step as a callback so its rollback is testable without an editor). It caches the bytes, then flushes the persisters and writes the `attachments` rows — an image it reaches may have been pasted seconds ago with its `blocks` row still on the debounce, and a row uploaded ahead of its block is dropped. One pass covers every URL route — the `![alt](url)` input rule, pasted markdown, and copying an image off a web page — instead of an async upgrade per route. It skips `data:`/`blob:` srcs and unstamped blocks, no-ops offline, and leaves the URL in place when a fetch fails.
  - **Both paths roll back a stored file that nothing ends up referencing** — a rejected insert slice, or a node that moved before the adopt pass could point at it. Nothing else would reclaim it: the persister's cascade needs a block row, and the orphan sweep only removes objects whose row is *gone*. In the adopt case a kept file is worse than a leak, since the image still carries its URL and the next pass would store another copy.

- **Page banner** — `src/lib/notes/banner.ts` + `src/components/notes/page/NotePageBanner.tsx`
  - A wide cover image above the page title. The bytes are an ordinary **page-owned attachment** (`attachFile(file, { pageId })`) and the page's `properties` JSON records which one it is: `banner` (the attachment id) and `bannerAlign` (the vertical crop as a percent, absent when centred). `attachments` has no role column, so this needs no schema change — and because the file is page-owned it syncs, appears in the details rail marked "Banner", cascades on a hard delete and survives a soft one, so a page restored from the Trash keeps its banner.
  - `banner.ts` holds the model: `readPageBanner` / `writePageBanner` (clearing both keys when removed), `attachBannerFile` — which validates with `isAllowed` itself, since `pickImageFiles` validates nothing — `attachBannerFromUrl` (through the same `fetchRemoteImage` proxy as bookmark previews), and `nextAlignFromDrag`, pure so the drag maths is tested without a pointer.
  - Two components, because the two states belong in different places: `NotePageBanner` renders above the title with hover controls (reposition / replace / remove), and `AddBannerChip` sits in the header's chip row — there is nothing to hover over when a page has no banner. `NotesEditorHeader` takes it as a `bannerSlot`, so the header stays unaware of banners.
  - **Replacing** a banner deletes the image it replaced (`bannerImageToDiscard` names the rule, since it decides a deletion) — by id, because a banner is stored against the *page* and clearing the page's attachments would take every file on it, and never when the same picture is chosen again, which the reuse-an-existing-image source makes possible. **Removing** a banner still only clears the property and keeps the file: "Remove banner" reads as taking it off the page, not as deleting a picture, and the file stays listed and labelled in the rail where it can be deleted deliberately. Deleting it from the rail clears the property in the same handler, and a banner whose row is gone renders nothing rather than a broken frame.
  - The gallery card shows it as a cover strip (`NoteCardCover` in `overview-views.tsx`), mounted only for a page that has one so a plain card runs no lookup — the same per-card cost `BookmarkCard` already pays through `useEntityImage`.
...
- The whole tracker UI lives in `src/components/tracker/TrackerWorkspace.tsx`, mounted by `src/app/tracker/layout.tsx`. Because it sits in the **layout**, it persists across view changes and **above the route loading boundary**, so switching views never unmounts the shell or flashes a route skeleton. `TrackerWorkspace` reads the view from `usePathname()` and navigates by pushing the sibling path (a `pendingView` render-time guard keeps the tab switch instant). **Each view is built on its first visit and kept mounted after**, shown and hidden rather than swapped: a year heatmap owns its own queries, so unmounting it threw them away and returning drew a full skeleton over data the app already had — on every switch. That is also why there is no crossfade between views; a native tab bar switches instantly, and the animated tab underline carries the cue. A view that stays mounted has to be told when it is off screen: both year grids portal their floating Insights button to `<body>`, where the panel's `hidden` cannot reach it, so they take an `active` prop that gates the portal and its dialog. A cold load is covered by `AppBootSkeleton` (view-shaped by pathname). This mirrors the notes layout-hoist pattern.
...
`src/app/day/[date]/page.tsx` — `/day/<yyyy-MM-dd>`, a cross-app destination like `/trash`, carrying its own `AppConfig` (`src/lib/shared/destinations.ts`, shared with its skeleton) rather than borrowing Tracker's. It answers "what happened on this day" across every app, led by the journal — the one thing here you come to write rather than read: mood and hours (`useTimeGrid` + `summarizeDay`), that day's grid column to paint (the same `TimeGrid` and `ActivityToolbar` as the week), tasks due and completed, events logged, what was saved (notes, bookmarks and quotes kept that day), the day's journal entry inline (`DailyJournalEntry`, which materializes its page on the first keystroke — the page passes `hasEntry` from `useJournalEntryDays`, without which a day that has been written on still opens on the empty prompt; that hook tests for *blocks*, not for the page, because linking a day creates its page and a day you have only mentioned should still read as unwritten), and what links to the day (`useBacklinks` on its journal page id, the anchor a `[[day]]` reference resolves against). `src/components/skeletons/DayLoadingSkeleton.tsx` is its boot and route fallback — the page's own shape, not Tracker's seven-row week.

Everything it lists is a row of `src/components/links/EntityRow.tsx` — the kind's icon from `REF_KIND_ICON` in the kind's accent, opening through `dispatchOpenEntity` exactly as an inline `[[ ]]` chip does, so a task opens its popup and a note navigates. Kinds that only navigate pass `href` as well, which keeps the row a real link. Use it on any surface that lists several kinds together rather than choosing icons there: a new kind then reaches every such list at once.
...
**⌘K recognises a date** — "sep 15", "2026-09-15", "yesterday", "today" — and offers it under *Go to* beside Graph and Trash. That recognition is `parseDayQuery` (`date-tokens.ts`), shared with the `[[` picker so both agree on what a date is: it fills in the current year (`new Date("sep 15")` lands in 2001) and demands a month name or digits around a separator first, because `new Date` otherwise reads "meeting notes 2026" as January. The Day surface's own header is a calendar picker, with a Today reset beside the arrows, and the dashboard's date opens today. A day wears a deeper green than Tracker's teal (`REF_KIND_SHADE` carries the darker shade) — near enough to read as time, far enough not to be mistaken for the tracker.
...
Two stores gained a field for it: `tasks.completed_at` (any edit moves `updated_at`, so it can't answer *when it was finished*) and a quote's `addedAt`, which bookmarks already had. Rows kept before either field existed carry none, so `useDayCaptures` windows on `COALESCE(NULLIF(json_extract(content,'$.addedAt'),''), updated_at)` — a block's `updated_at` is set on insert, so for anything never edited since it *is* the capture time, and the fallback stops applying the moment a row carries a real `addedAt`.
...
- `src/components/tracker/TimeGrid.tsx`
  ...
  - Clicking a cell writes or clears a time log entry; the Day column's name opens `/day/<key>`, so a row in the week is a way into that whole day
...
- **Stacking order**, so a layer added later doesn't land under one it should cover: dialog and drawer at 50, alert dialog at 60/61, the editor's `[[` menu at 80, and **toasts at 90** — a toast reports what just happened, so nothing that can raise one may hide it. An undo offered from inside a dialog and rendered behind it is worse than no undo, since the delete still went through
- **Every route has a face.** A `layout.tsx` per route sets the tab title (`"<Name> | Dash."`) and a `public/icon-<name>.svg` favicon, so a pinned tab says which app it is. The three cross-app destinations — a day, the graph, the trash — are no different: `src/lib/shared/destinations.ts` holds their `AppConfig`s. None is an app, so none belongs in the registry above or the switcher's app list, but each needs the identity the shared shell renders — and each is shared with its loading skeleton, so a route and its skeleton wear the same face
...
**Opening a screen offline.** `next.config.ts` precaches the app's shells (`APP_SHELLS`, ~6 KB gzipped each), so every section opens offline on a device that has never visited it — a screen cached only on visit would not open at all on a fresh install. The shells hold no rows, since every screen renders from local SQLite after hydration, so the list grows when an app is added and never when data is. Supplying `additionalPrecacheEntries` *replaces* the plugin's own `public/` scan rather than adding to it, hence `publicAssets()` beside it — without that the icons and web manifest silently leave the precache. `/tracker/[view]` names its three views with `generateStaticParams`, which makes them prerendered and precacheable rather than on-demand.

**What the precache is allowed to keep.** The service worker fetches every precache URL once, at install, and keeps the answer until that entry's revision changes — so a wrong answer, once stored, is permanent. Three rules keep one out:

- **Static files never meet the sign-in check.** `src/proxy.ts` excludes them from its matcher *by extension* (`svg`, `png`, `ico`, `json`, `txt`, fonts…), not by name, so a file added to `public/` is covered without being listed. A static file that goes through the check can be answered with the login page, and the precache would store that page under the file's URL. Nothing private lives in `public/`. `tests/shared/precache-guard.test.ts` pins which paths are checked and which aren't.
- **A redirect is refused, not followed.** `refuseRedirectedPrecache` (`src/lib/shared/precache-guard.ts`), passed as a precache plugin in `sw.ts`, rejects any response that arrived by redirect. Serwist otherwise follows it and stores the final page under the original URL; here the only redirect is the sign-in check, so following it means caching the login page as an icon or a whole shell, and a shell captured that way opens as the sign-in page offline. Rejecting one entry fails the install, which is the intent — the running worker stays in charge and the next update check retries. Supplying any `cacheWillUpdate` plugin switches off Serwist's own rejection of error responses, so the guard repeats it.
- **A known-bad copy is replaced by bumping `PUBLIC_ASSET_GENERATION`** in `next.config.ts`. A `public/` file's revision is its own hash folded with that constant, and an entry is never fetched again while its revision is unchanged — so nothing on a device can repair a bad copy of an unchanged file; a new revision makes every installed copy fetch it again.

To clear a bad entry on one device by hand, delete the precache under DevTools → Application → Cache Storage and reload. **Never "Clear site data"**: that also wipes IndexedDB and OPFS, which hold the local database and any file or tag that has not reached the server yet.

Data pages are deliberately not precached — one URL per note or per day is unbounded and would still miss the next one. They fall back to their section's shell (`fallbacks` in `sw.ts`): `/notes/<id>` → `/notes`, `/day/<date>` → `/day`, `/events/<id>` → `/events/_`. That works because none of those pages renders anything server-side — the notes route returns `null`, and the day and event surfaces read their date/id from `usePathname` rather than `useParams`, which would come from the shell's own payload and pin every URL to whatever the shell was built for — so the shell hydrates, the router sees the real URL, and the right thing renders from SQLite. `/events/[id]` prerenders the single id `_` as that stand-in; it resolves to no event and shows the not-found state, which is the honest answer for a URL nobody should land on directly.

A route's page must be a plain wrapper, not `export { X as default } from …`: Next cannot read segment config through a re-export, which silently cost `/events/[id]` its `generateStaticParams` until the wrapper was written out.

**Landing edits when the app goes away.** Writes are debounced before they reach SQLite (a second for a field, longer for a note body), so the newest keystrokes live only in a pending timer. `src/components/FlushOnHide.tsx`, mounted below PowerSync, flushes both queues on `visibilitychange → hidden` and `pagehide`. **Not `beforeunload`** — that fires when a *tab* closes, which is not how an installed app ends: a phone backgrounds it and the OS reclaims it later with no unload at all, and iOS standalone barely fires it. Anything relying on it loses whatever was mid-debounce, which surfaces as "my offline edits vanished" even though the loss happened on the way out, not on the way back. The per-page `beforeunload` handlers stay as a desktop backstop, where that event still fires first on a tab close.

**Picking up a deployment.** An installed app has no reload gesture — and `overscroll-behavior: none` removed the pull-to-refresh that was standing in for one — so a new version has to be applied deliberately. `src/app/sw.ts` sets `skipWaiting: false`, which makes Serwist listen for a `SKIP_WAITING` message and, more importantly, stops a new worker activating under a running page and leaving it asking for JS chunks the new deployment no longer has. `src/lib/shared/app-update.ts` is the client half and the single place a reload happens, so the flush of pending writes before it can't be skipped by one path and not another. Three ways in: `AppUpdateWatcher` (mounted in the layout, inside the toast provider) offers a toast when an update lands mid-use and applies one silently when you return after `AWAY_BEFORE_SILENT_UPDATE_MS` — never over unsaved work, since writes are debounced and a reload would drop an edit the user believes is saved; and Settings has a "Check for updates" that applies one or says you're current.
