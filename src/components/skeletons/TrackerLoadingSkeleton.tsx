            ...
            {Array.from({ length: 42 }).map((_, row) => (
              <div key={row} className="flex items-center gap-[3px]">
                ...
                {Array.from({ length: 24 }).map((_, h) => (
                  <div
                    key={h}
                    className={`h-[11px] w-[11px] rounded-[3px] animate-pulse ${((row * 24 + h) * 13 + row) % 6 === 0 ? "bg-muted" : "bg-muted/30"}`}
                  />
                ))}
              </div>
            ))}
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {Array.from({ length: 12 }).map((_, m) => (
          <div key={m} className="rounded-xl border border-border bg-card/50 p-3">
            <div className="mx-auto mb-2 h-3.5 w-10 animate-pulse rounded bg-muted" />
            <div className="grid grid-cols-7 gap-1">
              {Array.from({ length: 35 }).map((_, i) => (
                <div
                  key={i}
                  className={`mx-auto h-4 w-4 rounded-full animate-pulse ${(m * 35 + i) % 5 === 0 ? "bg-muted-foreground/15" : "bg-muted/40"}`}
                />
              ))}
            </div>
          </div>
