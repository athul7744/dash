/**
 * Reminder recurrence — pure, DB-free (like `src/lib/shared/capture.ts`) so the
 * test suite can exercise it without loading PowerSync. All date math is in the
 * LOCAL calendar (matching `dayNumber` in `daily-pick.ts`); occurrences advance
 * at local midnight.
 */
import { format } from "date-fns";

import { dayNumber } from "@/lib/shared/daily-pick";

export type ReminderSchedule =
  | { freq: "once"; date: string } //            yyyy-MM-dd
  | { freq: "weekly"; weekday: number } //        0=Sun … 6=Sat
  | { freq: "monthly"; day: number } //           1–31, clamped to month length
  | { freq: "yearly"; month: number; day: number }; // month 0–11, day 1–31 (clamped)

/** Just the fields the due-check needs — keeps this module free of the DB-bound Reminder type. */
export interface DueInput {
  schedule: ReminderSchedule;
  daysBefore: number;
  lastMaterializedKey: string | null;
}

const WEEKDAY_NAMES = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];

/** Midnight of a date's local calendar day. */
function startOfLocalDay(d: Date): Date {
  return new Date(d.getFullYear(), d.getMonth(), d.getDate());
}

/** Days in a given local month (monthIndex 0–11). */
function daysInMonth(year: number, monthIndex: number): number {
  return new Date(year, monthIndex + 1, 0).getDate();
}

/** `yyyy-MM-dd` for a date's local day — the per-occurrence idempotency token. */
export function occurrenceKey(date: Date): string {
  return format(date, "yyyy-MM-dd");
}

/**
 * The next occurrence date on or after `from` (local-day granularity), or null
 * when a `once` schedule is already in the past. Month/year days that overflow
 * the target month are clamped (e.g. the 31st → Feb 28/29).
 */
export function nextOccurrenceOnOrAfter(schedule: ReminderSchedule, from: Date): Date | null {
  const base = startOfLocalDay(from);

  switch (schedule.freq) {
    case "once": {
      const [y, m, d] = schedule.date.split("-").map(Number);
      const target = new Date(y, m - 1, d);
      return target.getTime() >= base.getTime() ? target : null;
    }
    case "weekly": {
      const diff = (schedule.weekday - base.getDay() + 7) % 7;
      return new Date(base.getFullYear(), base.getMonth(), base.getDate() + diff);
    }
    case "monthly": {
      let year = base.getFullYear();
      let month = base.getMonth();
      // At most two steps are ever needed (this month, then next), but loop a
      // few times defensively against clamping edge cases.
      for (let i = 0; i < 4; i++) {
        const day = Math.min(schedule.day, daysInMonth(year, month));
        const candidate = new Date(year, month, day);
        if (candidate.getTime() >= base.getTime()) return candidate;
        month += 1;
        if (month > 11) {
          month = 0;
          year += 1;
        }
      }
      return null;
    }
    case "yearly": {
      let year = base.getFullYear();
      for (let i = 0; i < 2; i++) {
        const day = Math.min(schedule.day, daysInMonth(year, schedule.month));
        const candidate = new Date(year, schedule.month, day);
        if (candidate.getTime() >= base.getTime()) return candidate;
        year += 1;
      }
      return null;
    }
  }
}

function ordinal(n: number): string {
  const rem100 = n % 100;
  if (rem100 >= 11 && rem100 <= 13) return `${n}th`;
  switch (n % 10) {
    case 1:
      return `${n}st`;
    case 2:
      return `${n}nd`;
    case 3:
      return `${n}rd`;
    default:
      return `${n}th`;
  }
}

/** A short human summary of a schedule, e.g. "Every Monday", "Monthly on the 1st". */
export function formatSchedule(schedule: ReminderSchedule): string {
  switch (schedule.freq) {
    case "once": {
      const [y, m, d] = schedule.date.split("-").map(Number);
      return `On ${format(new Date(y, m - 1, d), "PP")}`;
    }
    case "weekly":
      return `Every ${WEEKDAY_NAMES[schedule.weekday] ?? "day"}`;
    case "monthly":
      return `Monthly on the ${ordinal(schedule.day)}`;
    case "yearly":
      return `Yearly on ${format(new Date(2000, schedule.month, schedule.day), "MMM d")}`;
  }
}

/**
 * Decide whether a reminder's next occurrence should be materialized today.
 * Returns the occurrence + its key when `today` is within the lead window and
 * that occurrence hasn't been materialized yet; otherwise null. The pending-gate
 * (is the previous task still open?) is applied by the materializer, not here.
 */
export function dueOccurrence(reminder: DueInput, today: Date): { occurrence: Date; key: string } | null {
  const occurrence = nextOccurrenceOnOrAfter(reminder.schedule, today);
  if (!occurrence) return null;
  const key = occurrenceKey(occurrence);
  if (key === reminder.lastMaterializedKey) return null;
  if (dayNumber(today) >= dayNumber(occurrence) - reminder.daysBefore) {
    return { occurrence, key };
  }
  return null;
}
