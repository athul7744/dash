...
Data pages are deliberately not precached — one URL per note or per day is unbounded and would still miss the next one. They fall back to their section's shell (`fallbacks` in `sw.ts`): `/notes/<id>` → `/notes`, `/day/<date>` → `/day`. That works because neither page renders anything server-side — the notes route returns `null` and the day surface reads its date from `usePathname` (not `useParams`, which would come from the shell's own payload and pin every day to one date) — so the shell hydrates, the router sees the real URL, and the right thing renders from SQLite. `/events/[id]` has no equivalent yet; its detail page would need the same client-derived treatment first.
