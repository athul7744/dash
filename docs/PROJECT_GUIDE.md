...
- **Markdown / Logseq import** — `src/lib/notes/import/*` + `src/components/notes/page/ImportLogseqDialog.tsx`
  ...
  - **Writes are one transaction per file** (`run-import.ts` → `createNotePageFromBlockNodes` in `notes.ts`): page, blocks, edges and tags together, so a malformed file costs only itself. Nothing awaiting I/O runs inside a write lock. Block ids are stamped *before* anything is written (`stampBlockIds`), because an image's file is stored against the block that owns it — and a stored file no block references is unreclaimable, so a failed page rolls its attachments back. `properties.kind` must stay absent or the page is hidden from `/notes`, search, graph and trash.
...
- **Inline images** — `src/lib/notes/editor/image-insert.ts` / `image-adopt.ts` + `src/components/notes/editor/NoteImageView.tsx` + `src/hooks/use-attachment.ts`
  ...
  - `insertImageFiles` is the single path behind paste (`handlePaste`, only when the clipboard carries no text — see `clipboardImageFiles`), drop (`handleDrop`, positioned with `posAtCoords`), and the `/image` slash command (`custom: "image-picker"`, handled in `SlashMenuLayer` like the date picker). It **mints the block id first**, attaches the file to it, inserts the block carrying both ids, then calls `flushAllBlockDocumentPersisters()`. That order is load-bearing: because the `blocks` row lands immediately, removing the image produces a delete write and the persister's DELETE branch already drops the block's attachments — cleanup needs no code of its own. And because the file is stored before anything is inserted, a failed write leaves no broken node. One image ⇒ one block (the node is block-level and `block-normalize` enforces one content node per block), which is what makes the entity-wide `deleteEntityAttachments(blockId)` correct.
  - `ImageAdopt` is a debounced pass that finds images still pointing at an `http(s)` URL (`findAdoptableImages`, pure over the doc JSON), downloads them through `fetchRemoteImage`, and records the attachment id (`adoptImage`, which takes the "point the node at it" step as a callback so its rollback is testable without an editor). One pass covers every URL route — the `![alt](url)` input rule, pasted markdown, and copying an image off a web page — instead of an async upgrade per route. It skips `data:`/`blob:` srcs and unstamped blocks, no-ops offline, and leaves the URL in place when a fetch fails.
