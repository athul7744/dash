...
export function EventDetailSurface() {
  ...
  // From the path, not the route's params: one prerendered shell serves every
  // event id, which is what lets an event never opened online open offline (see
  // the fallbacks in sw.ts). Params would come from the shell's own payload and
  // pin every URL to the shell's id.
