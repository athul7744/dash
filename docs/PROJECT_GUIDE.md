...
- **Link preview cards** — `src/lib/notes/link-embed.ts` + `src/lib/notes/editor/link-embed-insert.ts` + `src/components/notes/editor/LinkEmbedNode.tsx`
  ...
  - A thumbnail is bounded to 640px before it is stored (`src/lib/storage/downscale.ts`). An og:image is routinely 2400px for a rail a tenth that wide, and the browser's one-pass reduction at that ratio uses its cheap filter — a banner carrying text comes out ragged. Stepping down by halves once, on the way in, is both sharper and a fraction of the bytes to sync. SVG and GIF are left alone (a vector already scales; a canvas keeps a GIF's first frame and silently drops the rest), and every failure path returns the original blob, so the worst case is the picture we already had. The re-encode changes format, and `extFor` trusts a file name's extension over the mime type, so the original extension is dropped rather than left lying about the bytes
