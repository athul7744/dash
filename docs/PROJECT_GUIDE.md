...
- **Link preview cards** — `src/lib/notes/link-embed.ts` + `src/lib/notes/editor/link-embed-insert.ts` + `src/components/notes/editor/LinkEmbedNode.tsx`
  ...
  - A card carries its own hover actions (open, copy, edit, remove). Editing offers the address and the title, and they behave differently on purpose (`mergeLinkEmbedEdit`): the title is a label, yours to set and kept; changing the address makes everything else stale, so description, host and thumbnail are re-read (`refetchLinkEmbed`) and only a title you typed yourself survives. A relabel touches no network. The replaced thumbnail is left in place, as a replaced page banner is — it is owned by the block and goes when the block does. The editor's link toolbar deliberately passes over one (`linkAt`): that toolbar edits a link *mark*, and a card is a node — there is no mark to change, and its rendered text spans a block, so the end position the toolbar infers from text length ran past the end of the document and `removeMark` threw
