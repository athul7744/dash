...
- **Link preview cards** — `src/lib/notes/link-embed.ts` + `src/lib/notes/editor/link-embed-insert.ts` + `src/components/notes/editor/LinkEmbedNode.tsx`
  ...
  - A card carries its own hover actions (open, copy, remove). The editor's link toolbar deliberately passes over one (`linkAt`): that toolbar edits a link *mark*, and a card is a node — there is no mark to change, and its rendered text spans a block, so the end position the toolbar infers from text length ran past the end of the document and `removeMark` threw
